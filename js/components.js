...
var components = {
	...
	messageFixes: {displayName: "Messaging changes", description: "Makes some changes to the messaging system.", exec: function() {
		...
		$("#id_submitbutton").click(function() {
			...
			if (isChanged) {
				...
				text = text + "<!-- Message parsed by Courses+ with parser revision 1 -->";
				$("#id_message").val(text);
			}
  ...
  <div class="container-fluid">
	...
	/*,
	optionsFooter: {displayName: "Add options link to the bottom of the page", description: "Adds a small link to the bottom of every Courses page that lets you easily reach the Options page. <br />You can always access this page by going to the hamburger menu in the top-right, clicking Tools->Extensions, and then 'Options' (under where it says Courses+)", exec: function() {
		var $link = $("<a>Courses+ Options</a>");
		//$link.attr("href", "chrome-extension://" + chrome.runtime.id + "/options.html");
		$link.add("coursesplus-footer");
		$link.click(function() {
			chrome.tabs.create({
		        url: "chrome://extensions/"
		    });
		});
		$("#page-footer").append($link);
	}, js: [], css: ["options.css"], runOn: "*", requires: []}*/
};
