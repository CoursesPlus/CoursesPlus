...
function loadWeather(location) {
	$.simpleWeather({
		...
		success: function(weather) {
			...
			cpal.storage.setKey("weather", wObj, function() {
				window.location.reload();
			});
		},
		error: function(error) {
			alert("Error while loading weather - " + error.message);
			window.location.reload();
		}
	});
}
...
$(document).ready(function() {
	...
	cpal.storage.getKey("weather", function(weatherObj) {
		$("#loading").hide();
		if (weatherObj == undefined) {
			$("#pickLocation").show();
		} else {
			$("#locationStr").text(weatherObj.city + ", " + weatherObj.region);
			$("#seeLocation").show();
		}
	});
	...
	cpal.storage.getKey("weatherUnits", function(units) {
		if (units == undefined) {
			cpal.storage.setKey("weatherUnits", "f", function() {

			});
		} else {
			$("#" + units + "_units").prop("checked", true);
		}
	});

	$("input[name=units]").change(function() {
		cpal.storage.setKey("weatherUnits", $("input[name=units]:checked").val(), function() {
